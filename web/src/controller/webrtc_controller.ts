import { actionType, updateSingleValue, receivedValues, disconnectedValues } from "slice/webrtc_slice";
...
const webRTCController = () => {
    ...
    const _peerStateChange = (e: Event) => {
        ...
        switch (peer.connectionState) {
            case "connected":
                store.dispatch(updateSingleValue({ value: true, type: actionType.isConnected }))
                break;
            case "disconnected":
                if (isConnected) {
                    close()
    ...
    // 본인 식별자 = 소켓 id
    const _socketConnected = async () => {
        store.dispatch(updateSingleValue({ value: socket.id, type: actionType.from }))
    ...
    const _updateUserList = (data: UserListModel) => {
        ...
        store.dispatch(updateSingleValue({ value: userList, type: actionType.userList }))
    }
    ...
    const turnOnMedia = async () => {
        ...
        try {
            ...
            store.dispatch(updateSingleValue({ value: true, type: actionType.onMedia }))
        } catch (e) {
            alert(e)
            return
        }
    ...
    const turnOffMedia = async () => {
        ...
        store.dispatch(updateSingleValue({ value: false, type: actionType.onMedia }))
    }
    ...
    const sendOffer = async () => {
        ...
        store.dispatch(updateSingleValue({ value: true, type: actionType.isCalling }))
        ...
        const offerModel: WebRTCModel = <WebRTCModel>{ to, from, audioOnly, offerSDP: offer.sdp, offerType: offer.type }
    ...
    const sendAnswer = () => {
        ...
        const answerModel: WebRTCModel = <WebRTCModel>{ to, answerSDP: answer.sdp, answerType: answer.type }

        socket.emit('answer', answerModel)

        store.dispatch(updateSingleValue({ value: false, type: actionType.receivedCalling }))
    }
